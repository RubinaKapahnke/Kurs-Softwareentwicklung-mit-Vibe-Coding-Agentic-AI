...
- **Markdown-Rendering repariert:** Migriert von `marked` zu `markdown-it` mit nativer GitHub Flavored Markdown (GFM) Unterstützung. `markdown-it` hat built-in Tabellen-Support, Links, Typografie und Zeilenumbrüche. DOMPurify konfiguriert für HTML-Tabellen (`<table>`, `<thead>`, `<tbody>`, `<tr>`, `<th>`, `<td>`) mit vollständigem CSS-Styling (Borders, Padding, Hover-Effekte).
