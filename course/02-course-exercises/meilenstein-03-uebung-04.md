...
Du prüfst, wie gut deine Dateien als KI-Kontext funktionieren, verbesserst dein PRD auf die fünf Standard-Abschnitte und erlebst direkt, wie sich ein vollstaendiger Kontext auf die Qualitaet der KI-Antwort auswirkt.
...
- [ ] Übung 3 (Prompt-Dateien) ist abgeschlossen
...
### 1. Dein PRD auf Vollständigkeit prüfen

Öffne `course/learners/<dein-name>/prd_<dein-name>.md` und prüfe, ob diese fünf Abschnitte vorhanden und ausgefüllt sind:

1. `## 1. Vision & Zielsetzung` - Ein Satz: Was ist das Ziel der App?
2. `## 2. Kernfunktionen (Features)` - Was soll die App können?
3. `## 3. Technische Leitplanken` - Welche Technologien werden eingesetzt?
4. `## 4. User Stories` - Als [Wer] möchte ich [Was], damit [Warum].
...
### 2. Fehlende Abschnitte mit KI ergänzen
...
```
Was soll meine App können? Nenne die wichtigsten Funktionen.
```
...
```
#prd_<dein-name>.md Was soll meine App können? Nenne die wichtigsten Funktionen.
```
...
```bash
git checkout -b UE-M3-04-<vorname>   # Erstellt einen neuen Branch für diese Übung
git add .
git commit -m "M3: PRD-Qualitaet verbessert und Kontext-Vergleich dokumentiert für <dein-name>"
...
- [ ] `prd_<dein-name>.md` enthält alle 5 PRD-Abschnitte (zumindest als Entwurf)
- [ ] `lernfortschritt_<dein-name>.md` enthält den Vorher-Nachher-Vergleich
...
- [ ] Ich kann benennen, welche Abschnitte ein vollständiges PRD haben sollte.
- [ ] Ich habe erlebt, wie sich die Qualitaet der KI-Antwort verbessert, wenn ich mein PRD als Kontext einbinde.
- [ ] Ich habe mindestens einen fehlenden oder duennen PRD-Abschnitt mit KI-Unterstützung verbessert.
