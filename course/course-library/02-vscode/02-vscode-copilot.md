...
| :--- | :--- | :--- |
| **Ask** | Fragen stellen, erklaeren lassen, diskutieren | Standard-Modus beim Oeffnen |
| **Edit** | Dateien direkt vom Chat aus bearbeiten lassen | Dropdown links vom Eingabefeld |
| **Agent** | KI fuehrt mehrstufige Aufgaben durch (liest, schreibt, erklaert) | Dropdown links vom Eingabefeld |

Fuer die meisten Aufgaben im Kurs nutzt du **Ask** oder **Agent**.

---

## Kontext mit `#` hinzufuegen

Die KI sieht standardmaessig nicht, welche Dateien du geoeffnet hast. Du musst ihr Kontext explizit geben.

**Datei als Kontext hinzufuegen:**
...
2. Eine Liste erscheint mit verfuegbaren Prompt-Dateien und eingebauten Befehlen
3. Eintrag auswaehlen → `Enter`
...
| `/fix` | Schlaegt einen Fix fuer ein Problem vor |
| `/tests` | Schreibt Tests fuer markierten Code |

**Prompt-Dateien aus `.github/prompts/`:** erscheinen ebenfalls in der `/`-Liste, wenn sie gueltiges Frontmatter haben.

> Falls eine Prompt-Datei nicht erscheint: Pruefe, ob sie in `.github/prompts/` liegt und das Frontmatter auf der ersten Zeile beginnt. Dann: `Strg+Shift+P` → `Developer: Reload Window`.
...
> Wann neuer Chat? Wenn du ein komplett neues Thema anfaengst und der bisherige Kontext eher stoert als hilft.

---

## Selbstcheck

- [ ] Ich kann Copilot Chat in VS Code oeffnen (Symbol oder Befehlspalette).
