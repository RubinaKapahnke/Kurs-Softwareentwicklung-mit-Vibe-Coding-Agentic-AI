<!-- AUTO-GENERATED FILE. DO NOT EDIT DIRECTLY. -->
<!-- Source: lerninhalte/lektion-03-github-account/lektion-inhalte.md -->

# Lektion 03: GitHub-Account anlegen

## Kapitel 1: Was ist GitHub?

GitHub ist eine Plattform im Web, auf der du Code speichern, versionieren und teilen kannst.

### Warum brauchen wir GitHub im Kurs?

1. Dein eigenes Übungs-Repository liegt auf GitHub.
2. Das gemeinsame Kurs-Repository liegt ebenfalls auf GitHub.
3. Die Trainerin sieht deine Commits und kann dir Feedback geben.

GitHub ist kostenlos nutzbar. Du brauchst nur einen Account.
Testzeile blabla

## Quiz: Testquiz
### Hinweis: Lies erst den Kontext
Das hilft bei der Auswahl.

#### Fokus
Achte auf den Begriff **Versionsverlauf**.

### Achtung: Test
Test2

### Info: Test
Test2

Frage: Wofür nutzt ihr GitHub im Kurs?
Hinweis: Wähle die beste Antwort.

- [ ] Falsche Antwort 1
- [ ] Falsche Antwort 2
- [x] Für Code, Versionierung und Zusammenarbeit
- [X] Nur für Videos

Erfolg: Richtig, genau darum geht es.
Fehler: Fast, schau dir den Kontext nochmal an.

### test
test 1

## Kapitel 2: Account erstellen

1. Öffne github.com.
2. Klick auf Sign up.
3. Gib E-Mail und Passwort ein.
4. Wähle einen Benutzernamen.
5. Bestätige deine E-Mail.
6. Fertig.

### Hinweise

- Der GitHub-Benutzername ist sichtbar.
- Nutze möglichst eine E-Mail-Adresse, die dauerhaft in deinem Besitz bleibt.
- Link: https://github.com/join

## Quiz: Kurze Verständnisfrage

Frage: Wozu dient GitHub im Kurs hauptsächlich?

- [ ] Als E-Mail-Dienst für Nachrichten an die Trainerin.
- [x] Als Plattform zum Speichern und Teilen von Code mit Versionsverlauf.
- [ ] Als Video-Lernplattform für Kursinhalte.

Erfolg: Richtig! GitHub ist deine Plattform für Code und Zusammenarbeit im Kurs.
Fehler: Nicht ganz. GitHub ist eine Plattform für Code - mit eingebautem Versionsverlauf und Zusammenarbeit.
